...
async function patchActions({ echoOnly }) {
    ...
    const entryFilePath = fs_1.default.existsSync(path_1.default.join(srcPath, 'Global.ts')) ? path_1.default.join(srcPath, 'Global.ts') : path_1.default.join(srcPath, 'Global.tsx');
    ...
    if (arr) {
        ...
        console.log(`Patch actions for ${cli_utils_1.chalk.cyan.underline(entryFilePath)}`);
        ...
        if (json === json2) {
            console.log('');
            console.log(cli_utils_1.chalk.green('There was no changes!\n'));
        }
        else {
            if (echoOnly) {
                ...
                console.log('');
                console.log(cli_utils_1.chalk.green(`✔ Has been patched!\n`));
            }
        }
    }
}
